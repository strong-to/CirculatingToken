...
- 对被引用的 `dataset/projects/*.json` 修正图片地址与简介文案，统一指向远程 OSS 资源（`https://miaocode-ai.oss-ap-southeast-1.aliyuncs.com/the4/...`），保证卡片展示与现有视觉一致。
- 将所有静态资源由本地 `/public` 改为远程 OSS 前缀 `https://miaocode-ai.oss-ap-southeast-1.aliyuncs.com/the4/`，彻底移除对本地文件路径的依赖。
